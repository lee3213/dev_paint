...
#include "define_local.h"
	int render_::P_Rendering(Mat & _src_ROI_canvas_clone,
		Mat & _before_ROI_canvas_clone, 
		cv::Mat & _changed_ROI_clone, 
		cv::Mat & _ing_ROI_canvas, 
		...
		Mat &_changed_ROI_canvas,
		unsigned char *_rst_canvas_ptr,
...

{
	...
	Mat hsv_v;
#endif
	unsigned char * ing_ROI_canvas_data = _ing_ROI_canvas.data;
	...
	unsigned char * changed_canvas_ROI_data = _changed_ROI_canvas.data;
	...
	int canvas_ROI_width = _changed_ROI_canvas.size().width;
	int canvas_ROI_height = _changed_ROI_canvas.size().height;
	int canvas_ROI_step1 = (int)_changed_ROI_canvas.step1();
	int canvas_ROI_channels = (int)_changed_ROI_canvas.channels();
	
	Mat src_ROI_clone_resized_8UC1;
	Mat a;
	resize(_src_ROI_canvas_clone, a, cv::Size(g_brush_thumbnail_size, g_brush_thumbnail_size));
	cv::cvtColor(a,src_ROI_clone_resized_8UC1, COLOR_RGB2GRAY);
	
	string f_name = "p" + to_string(_depth) + "/" + m_t_ + to_string(_depth) + "_t_" + to_string(_try_);
	if (tbrush_cnt < DEBUG_BRUSH_CNT) {
		debug_image( f_name+"_0src_ROI_ ", _src_ROI_canvas_clone);
		debug_image(f_name +"_1RsizROI_ ", src_ROI_clone_resized_8UC1);
		debug_image(f_name+ "_2bef_ROI_ ", _before_ROI_canvas_clone);
		debug_image(f_name  +"_3chg_ROI_", _changed_ROI_clone);
	}
	string br_tag="/br/"+f_name ;
	int brush_no = JudgementBrush(src_ROI_clone_resized_8UC1, /*added by cwlee*/astroke_depth,
		g_brush_thumbnail_size, g_brush_thumbnail_size, br_tag);
...
//#ifdef DEBUG_ALPHA
	if (tbrush_cnt < DEBUG_BRUSH_CNT) {
			...
			debug_image(f_name +"_6acc_ROI_", _changed_ROI_clone);
			tbrush_cnt++;
	}
//#endif
	...
	if (int_result == CHANGED_BETTER) {
		...
		for (int by = 0; by < _brush_area_h_size; by++)
		{
			...
			//	int canvas_ROI_clone_Index_3c = by *canvas_ROI_clone_step1 + bx * canvas_ROI_clone_channels;
				...
				int canvas_Index_3c = (canvas_centered_SrtPoint.y + by) *canvas_ROI_step1 + (canvas_centered_SrtPoint.x+bx) * canvas_ROI_channels;
				int gray_bIndex_1c = by*_brush_area_w_size + bx;
				int bIndex_3c = by *b_step1 + bx * b_channels;
					...
					if (Alpha < g_alpha_TH) {
						...
						p_poke(_rst_canvas_ptr, canvas_Index_3c, color_BGR_B, color_BGR_G, color_BGR_R);
		...
		if (tbrush_cnt < DEBUG_BRUSH_CNT) {
			...
			debug_image(f_name + "_6ing_ing_", _ing_ROI_canvas);
			debug_image(f_name + "_6ing_ROI_", _changed_ROI_canvas);
		}
	} // if changed better
