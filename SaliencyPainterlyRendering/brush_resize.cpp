...
*/
	...
	g_paint_try_scale[x_last_depth - 1] *= 2;
	...
	render_Brush *brush_resized;
		...
		for (int j=0; it != _brush_set.end(); it++,j++) {
			for (int i = 0; i < render_depth; i++) {
				...
				resize((*it)->brush_gray, brush_resized->brush_8UC1, Size(render_brush_size[i], render_brush_size[i]));
				//resize((*it)->bump, brush_resized->bump, Size(render_brush_size[i], render_brush_size[i]));
			//	resize((*it)->brush_gray, brush_resized->brush_gray, Size(render_brush_size[i], render_brush_size[i]));
			//	cvtColor(brush_resized->brush, brush_resized->brush_hsv, CV_BGR2HSV);
				...
				if (j == 0) {
					r_cout << i << ", " << j << " = " << setw(7) << cnz << " : " << setw(7) << render_brush_size[i] * render_brush_size[i] <<
						...
						debug_image("br2/th_" + to_string(i) + "_" + to_string(j), th);
				}
#ifdef  DEBUG_BR2
				string x = "br2/_";
				string y = x + m_tag_;
		...
		brush_set_image.setTo(255);
			...
			for (int j = 0; j < g_BrushNumber; j++) {
				...
				if ( j == 0 )
					debug_image("br2/" + to_string(j) + "_"+to_string(i), p_brush->brush_8UC1);
